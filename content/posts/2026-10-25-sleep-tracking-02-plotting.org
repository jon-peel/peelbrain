...
/This is a follow up to [[file:2026-01-20-sleep-tracking-01-org-setup.org][a previous post]]./
...
In a file named ~sleep.org~ I have this (it is the last header in the file, because this is not the first thing I want to see when I open this file, and it can get very long as the years go on).

#+CAPTION: a captured column view
#+begin_src org
,* Data
...
- ~:id~ :: this is a reference to the file we are using as a column view. It will use the column view defined
 at the top of that file to layout the columns here, so they don't need to be specified again.
...
- ~:match~ :: This is a filter. The column view will bring in levels one and two as well, so by filtering on only records with a move value I will only have the rows with actual data.  The decision to have this on ~move~ is random, it could be anything.

Pressing @@html:<kbd>C-c C-c</kbd>@@ on the ~#+BEGIN:~ line will load the table from ~diary.org~.
This must be done manually.  It is not done automatically like code blocks.
...
I a using R
[fn::: R is not a language I am very familiar with, so these were done with the assistance of an LLM. Especially with the tidyverse plugin, I did enjoy playing with the data and R is a langiage I am planning on putting a bit of time into learning properly.]
for this. \\

What I like about R is that it references table columns by name.
That means, when I add another column (e.g. when I added Year to this output) I didn't need to update the column indexes on all the other code blocks.


#+CAPTION: The ~clean-data~ code block.
#+begin_src org
#+NAME: clean-data
...
The name on top of this block is very important, because it allows the results of this block to be used as a table in another block.

* Visualization

I have a few more R code blacks that export charts. Here are a few of them.

** Histogram

This is a quick visualization of how many nights I have slept how many hours.
...
I want to see if I sleep a better or worse in different locations.
